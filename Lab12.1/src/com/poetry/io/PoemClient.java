...
public class PoemClient {
    ...
    private static void readPoem() {
        //  Instead of this excessive BufferedReader null-check
        try {
    ...
    */
    ...
    private static void writePoem() {
        String haiku = "Five syllables" +
                "Seven syllables" +
                "Five syllables";

        try {
            Files.writeString(Path.of("haiku1.txt"), haiku);
            
        } catch (IOException e) {
            e.printStackTrace();
        }
//       try (PrintWriter writer = new PrintWriter(new FileWriter("haiku.txt"))) {
//           writer.println("Morning chill in air");
//           writer.println("The world unburdened by growth");
//           writer.println("Winter is coming");
//       }
//       catch (IOException e) {
//           e.printStackTrace();
//
